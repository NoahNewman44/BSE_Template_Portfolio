...
| SD Card Adapter | Used to download code onto SD card from my laptop. | 7.99$ | <a href="https://www.amazon.com/dp/B081VHSB2V?ref=ppx_yo2ov_dt_b_fed_asin_title&th=1"> Link </a> |
...
Here is one of the prompts I used from claude. (Didn't follow directly just used as baseline)
...
Install Docker using the official installer from https://docs.docker.com/get-docker/ or run:
bashcurl -fsSL https://get.docker.com -o get-docker.sh && sh get-docker.sh

Create a config directory for Home Assistant:
bashmkdir ~/homeassistant && cd ~/homeassistant

Run the Home Assistant container:
bashdocker run -d --name homeassistant --privileged --restart=unless-stopped -e TZ=America/New_York -v ~/homeassistant:/config --network=host ghcr.io/home-assistant/home-assistant:stable

Wait 2-3 minutes for the container to fully start up
Open your web browser and go to http://localhost:8123 (or http://YOUR_SERVER_IP:8123)
Create your admin account when prompted (first user becomes the administrator)
Complete the onboarding by setting your home location, timezone, and preferred units
Allow Home Assistant to discover devices on your network when prompted
Start exploring the interface and adding your smart home devices
Access container logs if needed using: docker logs homeassistant
